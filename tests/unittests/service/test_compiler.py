...
def test_compiler(doc: GherkinDocument, registry: Tursu) -> None:
    ...
    assert (
        ...
        == textwrap.dedent(
            '''
    """User sign in with their own password"""
    ...
    def test_7_User_can_login(request: pytest.FixtureRequest, capsys: pytest.CaptureFixture[str], tursu: Tursu, app: Any):
        """User can login"""
        with TursuRunner(request, capsys, tursu, ['📄 Document: login.feature', '🥒 Feature: User sign in with their own password', '🔹 Rule: Successful login', '🎬 Scenario: User can login']) as tursu_runner:
    ...
    def test_11_User_can_t_login_with_wrong_password(request: pytest.FixtureRequest, capsys: pytest.CaptureFixture[str], tursu: Tursu, app: Any):
        ...
        with TursuRunner(request, capsys, tursu, ['📄 Document: login.feature', '🥒 Feature: User sign in with their own password', '🔹 Rule: Failed login attempts', "🎬 Scenario: User can't login with wrong password"]) as tursu_runner:
            tursu_runner.run_step('Given', 'a set of users:', app=app, data_table=[User0(username='Bob', password='dumbsecret'), User0(username='Alice', password='anothersecret')])
    ...
    def test_18_User_can_t_login_with_someone_else_username(request: pytest.FixtureRequest, capsys: pytest.CaptureFixture[str], tursu: Tursu, app: Any, username: str, password: str):
        ...
        with TursuRunner(request, capsys, tursu, ['📄 Document: login.feature', '🥒 Feature: User sign in with their own password', '🔹 Rule: Failed login attempts', "🎬 Scenario Outline: User can't login with someone else username"]) as tursu_runner:
            tursu_runner.run_step('Given', 'a set of users:', app=app, data_table=[User0(username='Bob', password='dumbsecret'), User0(username='Alice', password='anothersecret')])
